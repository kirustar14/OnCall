...
Use your tools if you need more information before deciding — you have three, each for a different \
kind of lookup, so pick the one that actually fits rather than defaulting to all of them or picking \
at random:
- search_patient_history: structured, exact lookups in Medplum — the actual allergy/medication/vital \
records on file for this patient, this case and (when a patient identity is known) prior ones. Use \
this when you need to know precisely what's documented.
- semantic_patient_search: fuzzy, semantic recall across everything ever said or recorded for this \
patient — including free-text notes — even if it's worded differently than what you're looking for \
(e.g. "any prior drug reactions" can surface a note that said "got hives after amoxicillin" without \
sharing a single keyword with your query). Use this for recall that structured lookups would miss, \
especially anything that would only exist as a note rather than a structured field.
- web_search: external clinical knowledge that has nothing to do with this specific patient — drug \
interactions, contraindications, dosing references, definitions.
...
- semantic_patient_search: fuzzy, semantic recall across everything ever said or recorded for this \
patient, including free-text notes, even worded differently than your query (e.g. "any prior drug \
reactions" can surface a note that said "got hives after amoxicillin"). Use this for recall a \
structured lookup would miss.
- web_search: external clinical knowledge that has nothing to do with this specific patient — drug \
safety, dosing limits, interactions, or facts you're not confident about or that may have changed \
since your training.
...
SEMANTIC_SEARCH_TOOL = {
    ...
    "description": (
        "Fuzzy, semantic recall across everything ever said or recorded for this patient — "
        "including free-text notes — from any of their cases, even if it's worded completely "
        "differently than your query. Finds things a structured/exact lookup would miss, "
        "especially anything that only ever existed as a note rather than a structured field "
        "(e.g. 'any prior adverse drug reactions' can surface a note that said 'got hives after "
        "amoxicillin' with no shared keywords)."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
            "query": {
                "type": "string",
                "description": "What you're trying to recall, in plain language, e.g. 'prior reaction to an antibiotic'",
            }
        },
        "required": ["query"],
        "additionalProperties": False,
...
async def search_patient_semantic(case: CaseState, query: str) -> str:
    ...
    across everything captured for this patient (by name if known, else scoped to
    just this case), including free-text notes that search_patient_history's
    structured Medplum lookup can't reach at all."""

    name = (case.case_details.get("name") or "").strip()
    results = await moss_client.search(query, patient_name=name or None, case_id=case.case_id, top_k=5)
